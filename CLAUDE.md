...
- **Window-close behavior**: `MainForm.OnFormClosing` cancels and hides the form on `CloseReason.UserClosing` (X button, Close button, Alt+F4). The only path to actually quit the process is the tray menu's Exit item. Other close reasons (system shutdown, etc.) pass through untouched. The debug window never auto-reopens; the user explicitly opens it from the tray menu or settings.
